...
2. **Numerical computation — significantly improved (R12).** The gradient-flow computation (§13.5) now uses arrested gradient flow with structural topology monitoring: when the toroidal core migrates toward the axis, the flow reverts to the last topology-preserving checkpoint and continues with reduced step size. This achieves $E = 265$ soliton units ($1.37\times$ BS minimum), a substantial improvement over the earlier unconstrained result ($E = 562$, $2.9\times$ BS). Topology is preserved (core at $\rho_0 = 1.35$), though $E_2/E_4 = 3.8$ indicates incomplete equilibration. The energy-weighted aspect ratio of the partially converged soliton is $A \approx 1.4$; the BS soliton has $A \approx 2$–$3$. Eq. (13.20) gives $C_2 \in [-0.62, -0.27]$ for the BS range, which *includes* QED's $-0.3285$. **Status: significant progress** — topology preservation demonstrated, energy within 37% of BS minimum. The definitive first-principles $C_2$ requires arrested Newton flow on a 3D grid.

### B. Significant Issues

3. **The $C_2$ estimate — strengthened but not definitive (R10/R12 update).** The three-tier analytical progression ($-2.47 \to -0.99 \to -0.33$) involved two uncertain parameters. The numerical computation provides a fourth route: Eq. (13.20) gives $C_2 = -\pi^2/(4A^2)$ directly from the soliton's aspect ratio, *without* the Barut self-interaction factor. For the Battye-Sutcliffe range $A \approx 2$–$3$, this yields $C_2 \in [-0.62, -0.27]$, bracketing the QED value. The arrested gradient flow (R12) achieves $E = 265$ ($1.37\times$ BS) with $A_{\text{partial}} \approx 1.4$; further convergence would increase $A$ toward the BS range. **The overall picture is persuasive**: two independent approaches (analytical shell model, geometric Eq. 13.20 at BS aspect ratio) both bracket the QED value, suggesting $C_2$ agreement is not purely coincidental.
...
**Effort: High. Impact: Very High. Status: Substantially improved (R12).**
The arrested gradient flow (§13.5) now achieves $E = 265$ ($1.37\times$ BS) with topology preserved, a major improvement over the earlier $E = 562$ ($2.9\times$ BS) with topology loss. The virial ratio ($E_2/E_4 = 3.8$) indicates incomplete equilibration; the remaining 37% energy gap likely requires arrested Newton flow on a 3D grid (as used by Battye-Sutcliffe) rather than 2D axisymmetric gradient descent. The existing code (`sim_fn_soliton_c2.py`) provides the infrastructure. A definitive computation would simultaneously determine the equilibrium aspect ratio and parameter-free $C_2$.
