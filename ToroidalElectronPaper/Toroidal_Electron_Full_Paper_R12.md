...
15. Physical Anomalies · 16. Lepton Generations · 17. Mass, Gravity & the Hierarchy · 18. Testing the Lamb Shift Prediction · 19. Experimental Predictions · 20. Standard Model Relations · 21. Conclusions · Appendix A: Speculative Extensions
...
The photon circulates at speed $c$ around the major circumference $2\pi R = \lambda_C$ (Compton wavelength). The circulation frequency is:

$$f = c/(2\pi R) = c/\lambda_C = m_e c^2/h \tag{3.2}$$

This equals the de Broglie frequency. The electron's rest energy $E = m_e c^2$ corresponds to a photon of wavelength $\lambda_C$ circulating at $c$ — the "photon in a box" analogy. The electromagnetic energy is trapped, giving the electron its inertial mass.
...
**The internal clock.** In the toroidal model, the circulating photon completes one orbit of the major circumference $2\pi R = \lambda_C$ in time $T_0 = \lambda_C/c = h/(m_e c^2)$. The associated angular frequency is:
...
**Resulting field structure.** The electric and magnetic field lines form $(1,1)$ torus knots — each winds once around both the major and minor circumferences simultaneously. Crucially, every E-field line is linked with every B-field line exactly once, yielding Hopf invariant $|H| = 1$. The Hopf invariant is computed via the helicity integral [13]:
...
**Method.** We solve the axially symmetric Faddeev-Niemi equations on a cylindrical grid using gradient flow. The field is parameterized as $\mathbf{n} = (\sin\Theta\cos(\varphi + \Phi),\; \sin\Theta\sin(\varphi + \Phi),\; \cos\Theta)$ with profile functions $\Theta(\rho, z)$ and $\Phi(\rho, z)$. The computational domain is a $200 \times 400$ grid covering $\rho \in [0.01, 3.99]$, $z \in [-3.99, 3.99]$ in soliton units ($\kappa_2 = \kappa_4 = 1$). The initial condition is the conformal Hopf map:

$$W_0 = \frac{2a\rho}{2az + i(\rho^2 + z^2 - a^2)}, \qquad \Theta = 2\arctan|W_0|, \qquad \Phi = -\arg(W_0) \tag{13.19}$$

with optimal scale $a = 0.5$, selected to minimize the initial energy. The gradient flow uses adaptive step-size control, $\rho$-weighted gradients (to suppress near-axis stiffness from $1/\rho^2$ terms), and per-point gradient clipping.

**Initial configuration.** The conformal Hopf map at $a = 0.5$ has energy $E = 8817$ soliton units ($45.8 \times$ the Battye-Sutcliffe minimum [51]), virial ratio $E_2/E_4 = 0.075$ (strongly Skyrme-dominated), and Hopf charge proxy $H \approx -0.96$. The toroidal core is located at $\rho_0 \approx 0.35$.

**Energy-weighted soliton shape.** The energy-weighted current distribution of the Hopf map has mean radius $\bar{\rho} = 0.37$, RMS width $\sigma_\rho = 0.13$, yielding an effective aspect ratio $\bar{\rho}/\sigma_\rho \approx 2.9$. This is consistent with the Battye-Sutcliffe finding [51] that the $|H| = 1$ soliton is a *fat torus* with aspect ratio $\sim$2–3 — qualitatively different from the thin-torus electron model with $R/r_e = 1/\alpha \approx 137$.

**Gradient flow results.** Energy decreases from 8817 to 562 ($2.9\times$ the BS reference of 192.5) over $\sim$7500 iterations, but the Hopf charge proxy degrades from $-0.96$ to $-0.38$, indicating topology loss. The virial ratio increases to $E_2/E_4 = 6.5$ (far from the equilibrium value of 1). This is a well-known difficulty: simple gradient descent does not preserve the Hopf invariant, because the topological charge is a non-local functional of the fields that is not respected by local gradient updates. Battye and Sutcliffe [51] used a specialised "arrested Newton flow" algorithm — effectively a momentum-damped second-order method that stabilises the topological sector — to obtain their energy minimum $E \approx 192.5$.
...
This generalises Eq. (14.6) to arbitrary aspect ratio. The energy-weighted distribution of the conformal Hopf map ($A \approx 2.9$) gives $C_2^{(\text{geo})} \approx -0.30$, within 9% of QED's $C_2 = -0.3285$. The Battye-Sutcliffe soliton's aspect ratio of 2–3 yields $C_2 \in [-0.62, -0.27]$, a range that *includes* the QED value. An aspect ratio of $A \approx 2.75$ gives $C_2 = -0.33$, matching QED to $\sim$0.5%.

| Aspect Ratio $A$ | Source | $C_2^{(\text{geo})}$ | Ratio to QED |
|---|---|---|---|
| $1/\alpha \approx 137$ | Thin-torus model | $-2.47$ | $7.5\times$ |
| $2.86$ | Energy-weighted Hopf map (this work) | $-0.30$ | $0.91\times$ |
| $2.75$ | Value matching QED | $-0.33$ | $1.00\times$ |
| $\sim$2–3 | Battye-Sutcliffe [51] | $-0.27$ to $-0.62$ | $0.8$–$1.9\times$ |

**Status.** The computation confirms the fat-torus shape of the $|H| = 1$ sector and demonstrates that the $C_2$ estimate is highly sensitive to the soliton's aspect ratio — the fat-torus value ($C_2 \approx -0.30$) is dramatically closer to QED than the thin-torus value ($C_2 \approx -2.47$). A definitive computation requires topology-preserving energy minimisation (arrested Newton flow or constrained optimisation), which would simultaneously determine the equilibrium aspect ratio and virial-balanced energy from first principles. The simulation code (`sim_fn_soliton_c2.py`) provides the infrastructure for such an extension.
...
| Numerical soliton (§13.5) | $-0.30$ | $0.91\times$ | Energy-weighted Hopf map, $A \approx 2.9$ |
...
| $C_2 = -0.3285$ from two-loop QED | Soliton cross-section + self-interaction (§14.2) | Semi-quantitative: $C_2 \approx -0.33$ (shell model) or $-0.30$ (numerical, §13.5) |
...
| Ps 1S-2S | 1233607222.2 MHz | Modified overlap | TBD | Post-laser-cooling era |
...
11. **Quarks and QCD:** Extend the framework to fractional charges and color confinement.
...
### A.6 Quantum Gravity Implications
...
[36] Gabrielse, G. et al. (2006). "New Determination of the Fine Structure Constant from the Electron g Value and QED." Phys. Rev. Lett. 97, 030802. (Electron size bounds from $g-2$.)
